...
command appears in the README only if this run executed it (with its real
outcome), a decision only if the debate recorded it, a timestamp only if it came
...
generated set is redacted through the same task snapshot the evidence export
uses, so no credential can reach it, and the files it wrote (and preserved) are
recorded as a `submission_documentation_generated` audit event and listed in
`FINAL_REPORT.md`. Because the documents are written before the result is
captured, they appear in the reviewable diff and in a persisted project; when
milestone commits are enabled they are left uncommitted, since a milestone
commit only ever records verified milestone work.
